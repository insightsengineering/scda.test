...
## Program Name:              tsflab02a
## R version:                 4.2.1
## Short Description:         Program to create tsflab02a: Subjects With ≥1 [Laboratory Category]
...
##                            -tsflab02agc General chemistry
##                            -tsflab02akf Kidney function
##                            -tsflab02alv Liver biochemistry
##                            -tsflab02alp Lipids
##                            -tsflab02ahm Hematology
...
get_abbreviation <- function(parcat3sel) {
  ...
  if (length(parcat3sel) == 1) {
    ...
    # the line below should be removed for a true study, global jjcs standards in DPS system does not include the abbreviation
    if (parcat3sel == toupper("General chemistry")) {
      abbr <- ""
    }
    #
...
### note : the same core tblid (TSFLAB02a) will be used for all, inside the core function build_result_parcat3 the proper abbreviation will be added
