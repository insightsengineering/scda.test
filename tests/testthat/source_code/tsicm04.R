...
## Program Name:              tsicm04.R
## R version:                 4.2.1
## junco version:             1.0
## Short Description:         Program to create tsicm04: Concomitant Medications by
##                            ATC Level X/Base Preferred Term, with x% frequency -
##                            Variant 2
...
# - Define whether or not to include optional top row (default=FALSE)
...
adcm <- adcm_jnj %>%
  filter(ONTRTFL == "Y") %>%
  select(USUBJID, ONTRTFL, all_of(atcvar), CMBASPRF)
...
if (length(adcm$ONTRTFL) != 0) {
  ...
  ################################################################################
  # Prune table to only keep those that meet x% criteria on the total column, but top row is not subject to pruning (if optionally displayed)
  ################################################################################
  result <- safe_prune_table(
    result,
    prune_func = bspt_pruner(
      fraction = 0.05,
      keeprowtext = "Subjects with >=1 concomitant medication",
      cols = c("Total")
    )
  )
}

# If optional top row is required but there is no data remove it row and display "No data to display" text
if (length(adcm$ONTRTFL) == 0) {
  result <- safe_prune_table(
    result,
