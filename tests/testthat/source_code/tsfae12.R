...
## Program Name:              tsfae12.R
## R version:                 4.2.1
## junco Version:             1.0
## Short Description:         Program to create tsfae12: AE table by SOC/PT - TEAEs
...
adae <- adae_jnj %>%
  ...
  select(USUBJID, TRTEMFL, AEACN, AEBODSYS, AEDECOD)
...
lyt <- lyt %>%
  analyze(
    ...
    extra_args = append(
      extra_args_rr,
      list(label = "Subjects with >=1 AE", NULL)
    )
  ) %>%
  split_rows_by(
    "AEBODSYS",
